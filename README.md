...
**전술적 의미**:
- ATA < 4° + 거리 152~914m → Gun WEZ 진입 (데미지 발생)
...
| `IsMerged` | `merge_threshold=500` m | 근접 교전 상황 |
...
| `ClosureRateAbove` | `threshold=50` m/s | 접근 속도 > 임계값 |
| `ClosureRateBelow` | `threshold=0` m/s | 접근 속도 < 임계값 (멀어지는 중) |
| `TurnRateAbove` | `threshold=5` °/s | 선회율 > 임계값 |
| `IsOneCircle` | - | 1-circle 선회 상황 |
| `IsTwoCircle` | - | 2-circle 선회 상황 |
| `EnergyDiffAbove` | `threshold=500` m | 에너지 차이 > 임계값 |
