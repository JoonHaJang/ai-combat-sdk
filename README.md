...
| 노드 | 기본값 | 설명 |
|------|--------|------|
| `BelowHardDeck` | `threshold_ft=3281` | 고도 < 임계값 |
| `EnemyInRange` | `max_distance_ft=16404` | 적이 지정 거리 이내 |
| `DistanceBelow` | `threshold_ft=9843` | 거리 < 임계값 |
| `DistanceAbove` | `threshold_ft=6562` | 거리 > 임계값 |
| `AltitudeAbove` | `min_altitude_ft=9843` | 고도 ≥ 지정값 |
| `AltitudeBelow` | `min_altitude_ft=3281` | 고도 ≤ 지정값 |
| `VelocityAbove` | `min_velocity_kts=389` | 속도 ≥ 지정값 |
| `VelocityBelow` | `max_velocity_kts=778` | 속도 ≤ 지정값 |
| `UnderThreat` | `aa_threshold_deg=120` | AA > 임계값 (정면 위협) |
| `ATAAbove` | `threshold_deg=60` | ATA > 임계값 (적이 측면/후방) |
| `ATABelow` | `threshold_deg=30` | ATA < 임계값 (적이 전방) |
| `IsOffensiveSituation` | - | OBFM: 공격 유리 상황 |
| `IsDefensiveSituation` | - | DBFM: 방어 필요 상황 |
| `IsNeutralSituation` | - | HABFM: 정면/고측면 대등 상황 |
| `IsMerged` | `merge_threshold_ft=1640` | 근접 교전 상황 |
| `EnergyHighPs` | `threshold_fts=0` | 잉여 에너지(Ps) > 임계값 |
...
| `ClosureRateAbove` | `threshold_kts=97` | 접근 속도 > 임계값 |
| `ClosureRateBelow` | `threshold_kts=0` | 접근 속도 < 임계값 (멀어지는 중) |
| `TurnRateAbove` | `threshold_degs=5` | 선회율 > 임계값 |
| `IsOneCircle` | - | 1-circle 선회 상황 |
| `IsTwoCircle` | - | 2-circle 선회 상황 |
| `EnergyDiffAbove` | `threshold_ft=1640` | 에너지 차이 > 임계값 |
...
| `ClimbTo` | `target_altitude_ft=19685` | 목표 고도로 상승 |
| `DescendTo` | `target_altitude_ft=13123` | 목표 고도로 하강 |
| `AltitudeAdvantage` | `target_advantage_ft=1640` | 적보다 지정 고도 우위 유지 |
| `DefensiveManeuver` | `critical_aa_threshold_deg=45`, `danger_aa_threshold_deg=90` | AA 기반 방어 기동 |
...
- type: Action
  name: Pursue
  params:
    close_range_ft: 2500      # 더 넓은 근거리 판정
    bearing_straight_deg: 3   # 더 정밀한 조준
    ata_lost_deg: 45          # 더 빠른 회전 반응
    far_range_ft: 5000        # 원거리 기준 확장

# 보수적 추적
- type: Action
  name: Pursue
  params:
    alt_gap_fast_ft: 300      # 더 큰 고도차에서만 급기동
    bearing_hard_deg: 80      # 더 큰 방위각에서만 급회전
```
