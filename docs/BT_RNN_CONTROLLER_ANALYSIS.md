# BT → RNN 제어기 분석 (새 엔진 v0.11 — 2026-06-02 전수실측 기준)

> **이 문서는 전부 새 엔진(v0.11) 정본이다.** 구엔진(v0.10)/H40b 본문은 제거됨
> (과거 "throttle 0.5 고정 / vel0~3 감속 / 입력 12차원" 등은 새 엔진에서 무효라 삭제).
> - **§10**: BT 액션공간 ↔ 실측 transfer-function (모든 전략의 기반)
> - **§15**: 파이프라인 단위·부호 레퍼런스 (혼동 방지 — 매번 참조)
> - **§16**: obs 방향성(부호) 분해능 (우리 선회방향 = roll_deg / 적 선회 재구성)
> - §11·§12 의 "구엔진" 수치만 **비교·근거용**(왜 −107kt 느린지, figure-8 승리 템플릿)으로 유효하며
>   각 표에 명시적으로 라벨링돼 있다. 그 외 구엔진 서술은 없다.

## 요약 (새 엔진 정본)

BT 고수준 액션 `[alt_idx, hdg_idx, vel_idx]` 가 RNN(GRU) 을 거쳐 JSBSim 조종면으로 내려간다.
핵심 제약(§10 전수실측):
- **감속 없음**(규칙1): vel0/1/2 전부 throttle≈0.40·335kt(바닥). 가속은 vel3(0.71·377kt)/vel4(0.85·398kt)뿐.
  "vel=1 감속" 류 명령은 전부 placebo.
- **선회 좌우 비대칭**(규칙2): 좌 −13°/s > 우 +10°/s → 진영 비대칭의 근원(§13.1).
- **수직축만 권한 살아있음**(규칙5): alt ±325ft/s. 수평 stern-chase 는 동일기체라 교착.

현재 성능: **seed0(진영A) 17/17(extender 3 제외)·무피탄**. 진영B(seed3)는 RNN 좌우 비대칭이 근본
원인이라 미해결(§13, RNN 재학습 영역).
