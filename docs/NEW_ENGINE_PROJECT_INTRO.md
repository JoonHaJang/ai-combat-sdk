...
> (기술 레퍼런스는 [NEW_ENGINE_OFFLINE_POLICY_METHODOLOGY.md](NEW_ENGINE_OFFLINE_POLICY_METHODOLOGY.md))
...
예: "거리를 좁혀라"는 추격엔 맞지만 방어엔 틀립니다. 그래서 **상황을 먼저 분리**하고,
각 상황의 정답을 따로 찾습니다.
...
- **[★우리 문제 적합성]**
  ...
  ③ ★오프라인 solver가 성립하려면 **제어가 결정론**이어야 함 — RNN의 비결정성은
...
- **[★우리 문제 적합성]** dogfight 승리는 **1° 단위 조준**과 **corner speed 유지**가 좌우 —
...
- **[★우리 문제 적합성]** dogfight의 교전 *유형*은 두 기체의 **속도벡터 정렬도(HCA)**로
  물리적으로 결정되고, 추격·선회는 **최적 속도가 반대**라 한 정책으로 못 섬 →
...
- **[★적합성]** dogfight는 여러 tactic이 **근소차로 비슷**한 상태가 많음(동률) →
