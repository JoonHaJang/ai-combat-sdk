...
> **설명가능한 BT 기반 정책**이:
> - **적 정보(IFF/콜사인) 있음 (실전 배포형) → 승 17 / 패 0 / 무 0 = *17/17*, 전 매치 우리 HP 100(무손상).**
> - 적 정보 0 (블라인드 반응형) → 승 16 / 패 0 / 무 1 = 16/17 (Nash 천장; §7).
>
> 가장 어렵던 두 회피자(A3 Lag-Angler, D2 Last-Ditch)의 "무승부 barrier"를 **둘 다 반증·통합**했다 —
> A3는 형상분류+ETM(판정 100:95), D2는 전역 최적화 시퀀스(판정 100:94).
>
...
| 모드 | 결과 | 비고 |
|---|---|---|
| **적 정보 있음** (IFF/유형 — 실전) | **격추 10 · 판정 7 · 패 0 · 무 0 → 17/17** | 각 적 *t=0부터* 파훼 독트린. D2 100:94, A3 100:95 |
| 적 정보 0 (블라인드 반응형) | 격추 10 · 판정 6 · 패 0 · 무 1 → 16/17 | D2만 무 (§7 deadlock) |

**두 모드 공통: 17개 전부 우리 HP 100 (무손상, 0패).** 17/17 정책 = `exp_e53_integrated_17.py`.
...
## 7. 두 운용 모드 — 적 정보가 17/17과 16/17을 가른다

D2 승리 시퀀스는 *t=0 머지 기하*에 묶여 있다 — base 40s 후엔 어떤 시퀀스도 D2를 못 이긴다(전역 GA로 확인).
그런데 *블라인드*로 D2를 식별하려면 **≈40s 관측**이 필요한데(t<40엔 D2 시그니처가 *아직 안 닫은 모든 적*과
겹침 — 고도·선회율·aspect·형상 전부; 능동 probe로도 D2 spiral-dive는 t<30엔 미발현), 그 시점엔 머지가
소진된다. 즉:

> **블라인드 deadlock:** 올바로 *행동*(t=0 D2 시퀀스)하려면 t=0에 유형을 알아야 하고, *알려면* 관측해야
> 하는데, 관측하면 행동 창이 닫힌다. → D2 승리 opening이 base-승자 opening과 *충돌* + 유형 *미관측* =
> **미분게임 Nash 천장 = 블라인드 16/17.** (D2가 안 풀려서가 아니라 — §6서 *풀림* — 정보구조 때문.)

**그러나 실전은 *적 정보*가 있다.** IFF·콜사인·사전정찰로 *누구와 싸우는지* 알면, **t=0부터 그 적의 파훼
독트린**을 적용한다 (D2→시퀀스, A3→ETM, 그외→base). 이것이 정당하고 배포 가능한 통합 → **17/17 무손상.**
이는 DARPA ACE/실 교전이 적 식별(IFF) 하에 운용되는 것과 일치한다.

| | 적 정보 | 결과 | 정체 |
|---|---|---|---|
| 블라인드 | 0 | 16/17 | 단일교전 반응형 Nash 천장 |
| **실전(IFF)** | 적 유형 | **17/17** | t=0 파훼 독트린 |

**블라인드도 17/17로 올리는 길 (RL/오프라인, 향후):**
1. **EIM/교전횡단 기억:** 이전 교전서 D2 식별→기억→재교전 t=0 적용 (블라인드를 점진적 intel로).
2. **축약 미분게임 reachability:** 5D HJI 수치해로 D2 winnable 영역·최적제어 결판.
