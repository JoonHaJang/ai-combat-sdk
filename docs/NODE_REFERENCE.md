...
| 노드 | 기본값 | 파라미터 | 설명 |
|-----|--------|---------|------|
| `EnemyInRange` | `max_distance=16404.0` | `max_distance` (ft) | 적이 지정 거리 이내 |
| `DistanceBelow` | `threshold=9843.0` | `threshold` (ft) | 거리 < 임계값 |
| `DistanceAbove` | `threshold=6562.0` | `threshold` (ft) | 거리 > 임계값 |

### 고도/속도 조건

| 노드 | 기본값 | 파라미터 | 설명 |
|-----|--------|---------|------|
| `AltitudeAbove` | `min_altitude=9843.0` | `min_altitude` (ft) | 고도 ≥ 지정값 |
| `AltitudeBelow` | `min_altitude=3281.0` | `min_altitude` (ft) | 고도 ≤ 지정값 |
| `BelowHardDeck` | `threshold=3281.0` | `threshold` (ft) | 고도 < 임계값 (Hard Deck 위반 위험) |
| `VelocityAbove` | `min_velocity=389.0` | `min_velocity` (kts) | 속도 ≥ 지정값 |
| `VelocityBelow` | `max_velocity=778.0` | `max_velocity` (kts) | 속도 ≤ 지정값 |

> ⚠️ **Hard Deck**: 고도 1640ft 이하 시 즉시 패배. `BelowHardDeck` + `ClimbTo`를 행동트리 최상단에 배치하세요.
...
| `InEnemyWEZ` | `max_distance=9843.0`, `max_los_angle=30.0` | `max_distance` (ft), `max_los_angle` (°) | 적 WEZ 내에 있음 |
...
| `SpecificEnergyAbove` | `threshold=16404.0` | `threshold` (ft) | He(고도+v²/2g) ≥ 임계값 |
| `IsMerged` | `merge_threshold=1640.0` | `merge_threshold` (ft) | 거리 < 임계값 (근접 교전) |
...
| `EnergyDiffAbove` | `threshold=1640.0` | `threshold` (ft) | 에너지 차이 > 임계값 |

#### 접근/선회율 조건

| 노드 | 기본값 | 파라미터 | 설명 |
|-----|--------|---------|------|
| `ClosureRateAbove` | `threshold=97.2` | `threshold` (kts) | 접근 속도 > 임계값 (양수=접근 중) |
| `ClosureRateBelow` | `threshold=0.0` | `threshold` (kts) | 접근 속도 < 임계값 (멀어지는 중 감지) |
...
| 노드 | 기본값 | 파라미터 | 설명 |
|-----|--------|---------|------|
| `ClimbTo` | `target_altitude=19685.0` | `target_altitude` (ft) | 목표 고도로 상승/하강 |
| `DescendTo` | `target_altitude=13123.0` | `target_altitude` (ft) | 목표 고도로 하강/상승 |
| `AltitudeAdvantage` | `target_advantage=1640.0` | `target_advantage` (ft) | 적보다 지정 고도 우위 유지 |
...
| 파라미터 | 기본값 | 설명 |
|---------|--------|------|
| `close_range` | 6562.0 ft | 근중거리 판정 |
| `very_close_range` | 4921.0 ft | 근거리 판정 |
| `far_range` | 13123.0 ft | 원거리 판정 (이상 시 급가속) |
| `mid_far_range` | 8202.0 ft | 중원거리 판정 |
| `alt_gap_fast` | 656.0 ft | 급기동 고도차 임계값 |
| `alt_gap_normal` | 328.0 ft | 일반기동 고도차 임계값 |
...
`relative_bearing_deg`와 `ata_deg` 기반으로 적의 미래 위치를 향해 선회합니다. Gun WEZ(±4°, 500~3000ft) 진입에 최적화.
...
| `alt_gap_threshold` | 492.0 ft | 고도 변경 임계값 |
...
| `GunAttack` | `relative_bearing_deg` 기반 정밀 조준 (Gun WEZ: ±4°, 500~3000ft) |
...
| `alt_gap_ft` | -49213 ~ 49213 ft | 고도 차이 (양수=적이 위) |
...
| `turn_rate_degs` | °/s, 양수 | 선회율 (g·tan(bank)/v 공식) |
...
| **WEZ** | Weapon Engagement Zone — Gun WEZ: ±4°, 500~3000ft |
| **BFM** | Basic Fighter Maneuvers — OBFM(공격)/DBFM(방어)/HABFM(정면) |
| **Hard Deck** | 최저 안전 고도 1640ft (위반 시 즉시 패배) |
