...
그러나 GunTracker·aggressive 는 여전히 무승부다. 이게 결정적이다 — 이제 에너지도 우위이고
거리도 519m 로 가까운데도 WEZ 진입이 0회다. 즉 무승부의 원인은 에너지도 거리도 아니라 각도다.
대칭 nose-to-nose 선회전에서 둘이 같은 선회율로 돌면 서로를 ATA<12 로 충분히 오래 못 겨눈다.
이 실험은 에너지를 원인에서 배제함으로써 V.4(추격-회피 미분게임의 무승부 균형)를 강하게 확증한다.

남은 레버는 각의 비대칭이다 — 동률 선회를 깨고 적 선회원 안으로 들어가는 진짜 lead-turn cutoff.
이것이 V.6 의 마지막 가설이며, 완전 대칭 상대엔 그조차 무승부가 게임의 값일 수 있다(V.4). 한편
에너지 바닥은 다른 적(EnergyFighter, defensive)을 개선하고 우리를 에너지 우위로 두므로 순이득이라
유지한다.
