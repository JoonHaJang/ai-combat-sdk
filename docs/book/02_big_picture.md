...
## 학습 목표
이 장을 마치면 다음을 할 수 있다.
- AIPILOT의 네 계층(관측, 의사결정, 제어, 물리)을 직관으로 설명한다.
- 승리 규칙(무장 사용 구역 피해 공식, 최저고도)을 수식으로 적는다.
- 물리 120Hz, 제어 20Hz, 의사결정 10Hz 의 시간 구조를 안다.


> 이 문서의 목적: 새 엔진(`new_match_engine/`)이 *무엇을, 왜, 어떻게* 하는지를
> 비전문가·신입도 따라올 수 있게 한 번에 잇는다. 논문 수준 설명은 `NEW_ENGINE_LQR_CONTROL_REPORT.md`
> 등에 있고, 이 문서는 전체 그림과 직관을 준다. 모든 용어는 처음 나올 때 정의한다.
>
...
1:1 공중전(dogfight)을 자율로 수행하는 AI 전투기 조종사를, *블랙박스 신경망이 아니라
...
> 모델 기반·결정론적·인용 가능한 제어기로 저수준 비행을 대체한다. 게인(gain, 제어 세기 숫자)이
...
각 화살표 위의 단어(Tactic, Setpoint, u)가 계층 간 인터페이스다. 하나씩 풀어보자.
...
`bt/tree_policy.py`의 TreePolicy가 if-then 스택으로 *상황 → 전술*을 고른다:
...
> 강화하면 다른 상황이 망가진다(실측 교훈). 그래서 상황을 나눠 각각 다른 전술을 쓴다.
...
여기가 새 엔진의 심장이다. 목표값(ψ\*,h\*,V\*) → 조종면 명령 u로 바꾼다. 두 겹(cascade)이다:
...
- LQR(선형 2차 조절기): 비행기 운동을 운영점 근처에서 *직선처럼 근사(선형화)*한 뒤,
  "추종 오차와 조종면 사용량을 동시에 최소화"하는 최적 게인 K를 수학으로 푼다
...
> 의 4계층으로 풀고, 각 계층이 *설명·증명 가능*하며, bridge로 기존 시스템에 그대로 끼운다.

---

## 연습문제
1. 네 계층을 위에서 아래로 적고, 각 계층의 입력과 출력을 쓰라.
2. 무장 사용 구역 피해율 공식을 적고, 최대 피해가 나는 조건을 구하라.
3. 의사결정이 1초에 몇 번 일어나는지와 그 이유를 설명하라.
