...
class HUD_Aircraft:
	...
	def update(cls, Main, aircraft: Aircraft, targets):
		...
		HUD_EnemyMarkers.update(Main, aircraft, targets)

		if not Main.satellite_view:
			HUD_MachineGun.update(Main, aircraft)


class HUD_EnemyMarkers:
	"""적기마다 붉은 사각형 박스와 ENEMY 텍스트를 화면에 표시"""
	...
	def init(cls, resolution: hg.Vec2):
		cls._sprites = []
		for i in range(cls._max_enemies):
			spr = Sprite(64, 64, "sprites/target_sight.png")
			s = (40 / 1600 * resolution.x) / spr.width
			spr.set_size(s)
			cls._sprites.append(spr)

	@classmethod
	def update(cls, main, aircraft, targets):
		i = 0
		for target in targets:
			if i >= cls._max_enemies:
				break
			if target.wreck or not target.activated:
				continue
			if target.nationality == aircraft.nationality:
				continue
			if target.type != Destroyable_Machine.TYPE_AIRCRAFT:
				continue
			mat, pos, rot, aX, aY, aZ = target.decompose_matrix()
			p2D = main.get_2d_hud(pos)
			if p2D is None:
				continue
			red = hg.Color(1, 0.1, 0.1, 0.9)
			# 적기 위치에 붉은 박스 스프라이트
			spr = cls._sprites[i]
			spr.set_position(p2D.x, p2D.y)
			spr.set_color(red)
			main.sprites_display_list.append(spr)
			# 적기 위 상단에 "ENEMY" 텍스트
			x = p2D.x / main.resolution.x - 20 / 1600
			y = p2D.y / main.resolution.y - 42 / 900
			Overlays.add_text2D("ENEMY", hg.Vec2(x, y), 0.014, red, main.hud_font)
			i += 1
