    ...
    def update_kinetics(self, dts):
        if self.activated:
            ...
            if not self.flag_custom_physics_mode:
                ...
                # Collisions (only in normal physics mode)
                mat = self.update_collisions(matrix, dts)
            else:
                # Custom physics: use matrix directly, skip collisions
                mat = matrix
...
class Aircraft(Destroyable_Machine):
    ...
    def __init__(self, name, model_name, scene, scene_physics, pipeline_ressource: hg.PipelineResources, instance_scene_name, nationality, start_position, start_rotation):
        ...
        self.thrust_level = 0
        self.jsbsim_bridge = None   # JSBSim bridge (Option B demo mode)
        self._pending_bridge = None  # Warmed-up bridge waiting to attach (main-thread safe)
    ...
    # ------------------------------------------------------------------
    # JSBSim bridge (Option B demo mode)
    # ------------------------------------------------------------------

    def detach_jsbsim(self):
        """Release JSBSim bridge resources before DF2 shutdown to prevent segfault."""
        if self.jsbsim_bridge is not None:
            self.jsbsim_bridge.destroy()
            self.jsbsim_bridge = None
        self._pending_bridge = None

    def attach_jsbsim(self, bridge):
        """Attach a JSBSimBridge and switch to custom physics mode.

        Once attached, update_kinetics() will step the bridge each frame
        and apply JSBSim state to this aircraft's scene node.
        DF2's internal physics is completely bypassed.
        """
        self.jsbsim_bridge = bridge
        self.set_custom_physics_mode(True)
        print(f"[Machines] attach_jsbsim: {self.name} -> custom_physics=True, bridge_init={bridge._initialized}")
        if bridge._initialized:
            mat12, v3 = bridge.step(0.0)
            self._apply_bridge_state(mat12, v3)
            print(f"[Machines] attach_jsbsim: initial pos=({mat12[9]:.1f},{mat12[10]:.1f},{mat12[11]:.1f})")

    def _apply_bridge_state(self, mat12, v3):
        """Apply JSBSimBridge output to DF2's custom kinetics.

        mat12: Harfang column-major 3x4 flat list [12 floats]
        v3:    Velocity vector [vn, vu, ve] m/s
        Row-packing identical to network_server.py update_machine_kinetics
        handler (lines 288-291) which is proven correct.
        """
        import math as _math
        if any(_math.isnan(n) for n in mat12 + list(v3)):
            return
        mat = hg.TranslationMat4(hg.Vec3(0, 0, 0))
        hg.SetRow(mat, 0, hg.Vec4(mat12[0], mat12[3], mat12[6], mat12[9]))
        hg.SetRow(mat, 1, hg.Vec4(mat12[1], mat12[4], mat12[7], mat12[10]))
        hg.SetRow(mat, 2, hg.Vec4(mat12[2], mat12[5], mat12[8], mat12[11]))
        self.set_custom_kinetics(mat, hg.Vec3(v3[0], v3[1], v3[2]))
    ...
    def update_kinetics(self, dts):
        ...
        # Option B: attach pending bridge from main thread (Harfang is not thread-safe)
        if self._pending_bridge is not None and self._pending_bridge._initialized:
            print(f"[Machines] update_kinetics: picking up _pending_bridge for {self.name}")
            self.attach_jsbsim(self._pending_bridge)
            self._pending_bridge = None

        # Option B: JSBSim bridge steps physics and drives position/attitude
        if self.jsbsim_bridge is not None and self.jsbsim_bridge._initialized:
            mat12, v3 = self.jsbsim_bridge.step(dts)
            self._apply_bridge_state(mat12, v3)
            # Sync visual effects from actual JSBSim throttle state
            jsb_thrust = self.jsbsim_bridge.get_thrust_level()
            self.thrust_level = jsb_thrust
            if self.jsbsim_bridge.get_post_combustion_active():
                self.activate_post_combustion()
            else:
                self.deactivate_post_combustion()
