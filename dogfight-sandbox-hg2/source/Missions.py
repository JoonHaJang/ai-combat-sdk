...
try:
	from jsbsim_bridge import JSBSimBridge
	_JSBSIM_BRIDGE_AVAILABLE = True
except ImportError as e:
	print(f"[Missions] JSBSimBridge not available: {e}")
	_JSBSIM_BRIDGE_AVAILABLE = False
...
# ============================ Client / Server mode
	...
	def network_mode_setup(cls, main):
		...
		_default_jsb_data = _os.path.abspath(
			_os.path.join(_script_dir, "..", "..", "src", "simulation", "envs", "JSBSim", "data"))
		try:
			jsbsim_data_path = _os.path.abspath(script_parameters.get("jsbsim_data_path", _default_jsb_data))
			jsbsim_model = script_parameters.get("jsbsim_model", "f16")
			jsbsim_enabled = script_parameters.get("jsbsim_bridge_enabled", False)
		except NameError:
			jsbsim_data_path = _default_jsb_data
			jsbsim_model = "f16"
			jsbsim_enabled = False
		...
		# Option B: JSBSim 브리지 인스턴스화 (warmup은 백그라운드 스레드로 — DF2 루프 블로킹 방지)
		if _JSBSIM_BRIDGE_AVAILABLE and jsbsim_enabled:
			import threading as _threading
			print(f"[Missions] Attaching JSBSim bridges (model={jsbsim_model}, data={jsbsim_data_path})")

			_warmup_threads = []

			def _warmup(ac, bridge):
				print(f"[Missions] _warmup START for {getattr(ac, 'name', '?')}")
				bridge.reset(**bridge._init_kwargs)
				ac._pending_bridge = bridge  # main thread picks this up in update_kinetics()
				print(f"[Missions] _warmup DONE for {getattr(ac, 'name', '?')} -> _pending_bridge set")

			for ac in main.players_allies:
				bridge = JSBSimBridge(jsbsim_model, jsbsim_data_path,
				                      origin_lat=60.0, origin_lon=120.0,
				                      df2_origin=[0.0, 4572.0, -503.0])
				bridge._init_kwargs = dict(lat=60.0, lon=120.0 - 503.0/111000.0,
				                           alt_m=4572.0, heading_deg=180.0, speed_mps=244.0)
				t = _threading.Thread(target=_warmup, args=(ac, bridge), daemon=False)
				t.start()
				_warmup_threads.append(t)
			...
			for ac in main.players_ennemies:
				bridge = JSBSimBridge(jsbsim_model, jsbsim_data_path,
				                      ...
				                      df2_origin=[0.0, 4572.0, 503.0])
				bridge._init_kwargs = dict(lat=60.0, lon=120.0 + 503.0/111000.0,
				                           alt_m=4572.0, heading_deg=0.0, speed_mps=244.0)
				t = _threading.Thread(target=_warmup, args=(ac, bridge), daemon=False)
				t.start()
				_warmup_threads.append(t)

			# Store threads so game loop can check completion (non-daemon = clean shutdown)
			main._jsbsim_warmup_threads = _warmup_threads
		elif jsbsim_enabled:
			print("[Missions] WARNING: jsbsim_bridge_enabled=true but JSBSimBridge import failed")
		...
		# IA 없이도 자동 록온이 동작하도록 타겟팅 장치에 직접 타겟 설정
		# (IAControlDevice.activate()가 호출될 때만 search_target()이 실행되기 때문)
		for ac in main.players_allies + main.players_ennemies:
