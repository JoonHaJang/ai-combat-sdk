...
def stop_server():
	...
	server_log += msg
	print(msg)
	# Wait for JSBSim warmup threads to finish before destroying FDM
	warmup_threads = getattr(main, '_jsbsim_warmup_threads', [])
	for t in warmup_threads:
		if t.is_alive():
			t.join(timeout=5.0)
	main._jsbsim_warmup_threads = []
	# Release JSBSim FDM references before Python/Harfang shutdown (prevents segfault)
	for machine in list(main.destroyables_items.values()):
		if getattr(machine, 'detach_jsbsim', None) is not None:
			machine.detach_jsbsim()
...
def server_update():
	...
	while flag_server_running:
		try:
			...
			# Unpause JSBSim bridges on client connect (prevent pre-connection crash)
			for machine in list(main.destroyables_items.values()):
				bridge = getattr(machine, 'jsbsim_bridge', None)
				if bridge is not None:
					bridge._paused = False
					print(f"[network_server] Unpaused JSBSim bridge for {machine.name}")
			while flag_server_connected:
				answ = socket_lib.get_answer()
				if answ is None:
					print("network_server.py - client disconnected (recv returned None)")
					flag_server_connected = False
					main.flag_client_connected = False
					break
				answ.decode()
			...
			import traceback
			print("network_server.py - server_update ERROR: " + str(e))
			traceback.print_exc()
			flag_server_connected = False
			main.flag_client_connected = False
			main.flag_client_update_mode = False
			main.set_renderless_mode(False)
			msg = "Socket closed: " + str(e)
			server_log += msg
			print(msg)
...
def update_machine_kinetics(args):
	if flag_print_log:
		print("update_kinetics: " + args["machine_id"])
	machine = main.destroyables_items[args["machine_id"]]
...
def set_plane_thrust(args):
	...
	machine = main.destroyables_items[args["plane_id"]]
	if getattr(machine, 'jsbsim_bridge', None) is not None:
		# Option B: DF2 [0,1] -> JSBSim [0,0.9]
		machine.jsbsim_bridge._pending_controls['throttle'] = \
			max(0.0, min(0.9, float(args["thrust_level"]) * 0.9))
		# Thrust is sent last per step -> mark dirty to trigger JSBSim step
		machine.jsbsim_bridge._controls_dirty = True
	else:
...
def set_plane_pitch(args):
	...
	machine = main.destroyables_items[args["plane_id"]]
	if getattr(machine, 'jsbsim_bridge', None) is not None:
		machine.jsbsim_bridge._pending_controls['elevator'] = \
			max(-1.0, min(1.0, float(args["pitch_level"])))
	else:
...
def set_plane_roll(args):
	...
	machine = main.destroyables_items[args["plane_id"]]
	if getattr(machine, 'jsbsim_bridge', None) is not None:
		machine.jsbsim_bridge._pending_controls['aileron'] = \
			max(-1.0, min(1.0, float(args["roll_level"])))
	else:
...
def set_plane_yaw(args):
	...
	machine = main.destroyables_items[args["plane_id"]]
	if getattr(machine, 'jsbsim_bridge', None) is not None:
		machine.jsbsim_bridge._pending_controls['rudder'] = \
			max(-1.0, min(1.0, float(args["yaw_level"])))
	else:
