...
class IsHighEnergy(_CondBase):
    """Smoothed 에너지 우위 (H-E1d, 2026-04-15).

    이전: instantaneous e_diff > threshold → tick 단위 oscillation noise
    수정: window_size tick buffer smoothed average (IsChaseStale time-scale 일치)
    """
    TUNABLE_PARAMS = {
        "energy_diff_min_ft": {"type": "cont", "range": (0, 5000), "default": 1000},
        "window_size": {"type": "disc", "choices": [10, 20, 30, 50], "default": 30},
    }

    def __init__(self, name="IsHighEnergy", energy_diff_min_ft=1000.0, window_size=30):
        super().__init__(name)
