"""Paper figures — docs/figures/ fig1~fig8 PNG. Real-data(trajectory/D2-seq/results)+schematic. All labels in English.
...
from matplotlib.patches import FancyArrowPatch, FancyBboxPatch, Wedge, Arc
import numpy as np
...
def fig1_architecture():
    ...
    for x, t, c in boxes:
        ax.add_patch(FancyBboxPatch((x, 1.5), 1.7, 1.4, boxstyle="round,pad=0.05", fc=c, ec="#444", lw=1.3))
    ...
    ax.text(9.65, 0.6, "JSBSim physics 120Hz\n-> judge (WEZ: ATA<12 deg &\n500-3000ft -> DAMAGE)", ha="center", va="center", fontsize=8.5)
    ...
    ax.text(4.5, 0.35, "feedback (next obs)", color=BLUE, fontsize=8.5, ha="center")
    ax.set_title("Fig 1.  Four-layer explainable BT policy", fontsize=12, loc="left")
...
def fig2_wez_geometry():
    ...
    ehead = np.array([0.85, 0.53]); ehead /= np.linalg.norm(ehead)
    thead = np.array([-0.2, 0.98]); thead /= np.linalg.norm(thead)
    los = enm - ego; losn = los / np.linalg.norm(los)
    ax.plot([ego[0], ego[0] + 2.2*ehead[0]], [ego[1], ego[1] + 2.2*ehead[1]], color=BLUE, lw=2)
    ax.annotate("our velocity", ego + 2.3*ehead, color=BLUE, fontsize=9)
    ax.plot([enm[0], enm[0] + 2.0*thead[0]], [enm[1], enm[1] + 2.0*thead[1]], color=RED, lw=2)
    ax.annotate("enemy velocity", enm + 2.0*thead + np.array([-1.6, 0.1]), color=RED, fontsize=9)
    ax.plot([ego[0], enm[0]], [ego[1], enm[1]], color="#444", ls="--", lw=1.3)
    ax.annotate("LOS (line of sight)", (ego+enm)/2 + np.array([0.1, -0.5]), fontsize=9, color="#444")
    a0 = math.degrees(math.atan2(ehead[1], ehead[0]))
    ax.add_patch(Wedge(ego, 7.5, a0-12, a0+12, fc=GREEN, alpha=0.15))
    ax.annotate("WEZ cone\nATA<12 deg", ego + 4.5*ehead + np.array([-0.2, 1.0]), color=GREEN, fontsize=9)
    ...
    ta = math.degrees(math.atan2(-thead[1], -thead[0])); la = math.degrees(math.atan2(-losn[1], -losn[0]))
    ...
    ax.text(ego[0]-0.1, ego[1]-0.5, "ego (us)", color=BLUE, fontsize=9, ha="center")
    ax.text(enm[0]+0.1, enm[1]+0.5, "enm (enemy)", color=RED, fontsize=9, ha="center")
    ax.set_title("Fig 2.  Engagement geometry - ATA, AA, LOS, WEZ", fontsize=11.5, loc="left")
...
def fig5_etm_concept():
    ...
    th = np.linspace(-0.3, 1.5, 60); R = 3.0; cx, cy = 2.0, 3.2
    arc = np.stack([cx + R*np.cos(th + 3.6), cy + R*np.sin(th + 3.6)], 1)
    ax.plot(arc[:, 0], arc[:, 1], color=RED, lw=2, label="enemy coordinated-turn arc (predicted)")
    now = arc[15]; fut = arc[40]
    ax.plot(*now, "o", color=RED, ms=10); ax.annotate("enemy now", now+np.array([0.1, -0.4]), color=RED, fontsize=9)
    ax.plot(*fut, "o", color=ORANGE, ms=10); ax.annotate("enemy at t+tau (ETM)", fut+np.array([0.1, 0.2]), color=ORANGE, fontsize=9)
    ax.plot(*ego, "^", color=BLUE, ms=14); ax.annotate("us", ego+np.array([-0.1, -0.4]), color=BLUE, fontsize=9, ha="center")
    ax.add_patch(FancyArrowPatch(ego, now, arrowstyle="-|>", mutation_scale=15, color="#888", ls="--"))
    ax.text(0.5, 1.2, "aim at current pos\n-> enemy escapes (lag)", color="#888", fontsize=8.5)
    ax.add_patch(FancyArrowPatch(ego, fut, arrowstyle="-|>", mutation_scale=15, color=BLUE, lw=2))
    ax.text(2.0, 0.6, "ETM: aim at predicted pos\n-> leads the evasion", color=BLUE, fontsize=8.5)
    ax.set_xlim(-1.5, 6); ax.set_ylim(-0.5, 6.5); ax.set_xticks([]); ax.set_yticks([]); ax.grid(False)
    ax.set_title("Fig 5.  ETM - lead the evasion via enemy trajectory prediction", fontsize=11.5, loc="left")
...
def fig8_deadlock():
    ...
    ax.text(20, 2.7, "observation window\n(type ID needs ~40s)", ha="center", fontsize=9, color=BLUE)
    ax.text(120, 2.7, "action window", ha="center", fontsize=9, color=GREEN)
    ax.add_patch(FancyBboxPatch((2, 1.4), 28, 0.5, boxstyle="round,pad=0.02", fc=ORANGE, alpha=0.5, ec=ORANGE))
    ax.text(16, 1.65, "D2 win window (needs t=0 merge)", ha="center", fontsize=8.5)
    ax.axvline(40, color="#444", ls="--"); ax.text(41, 0.55, "t~40s: type identifiable\n(but action window already closed)", fontsize=8.5)
    ax.add_patch(FancyArrowPatch((40, 1.0), (15, 1.4), arrowstyle="-|>", mutation_scale=14, color=RED, ls=":"))
    ax.text(52, 0.25, "deadlock: to act you must know the type at t=0; to know it you must observe; observing closes the window", fontsize=8.5, color=RED)
    ax.set_yticks([]); ax.set_xlabel("engagement time t (s)")
    ax.set_title("Fig 8.  Blind observe-act deadlock - D2 win window vs ID delay (Nash ceiling = 16/17)", fontsize=11, loc="left")
...
def fig7_results():
    ...
    res = ["K"]*10 + ["J"]*5 + ["J", "J"]
    colors = [RED if r == "K" else ORANGE for r in res]
    colors[-2] = PURPLE; colors[-1] = PURPLE
    fig, ax = plt.subplots(figsize=(11, 4.0))
    x = np.arange(len(opp))
    ax.bar(x, [100]*len(opp), color=BLUE, alpha=0.25, label="our HP = 100 (undamaged)")
    ax.bar(x, ehp, color=colors, label="enemy remaining HP")
    ...
    ax.set_ylabel("HP"); ax.set_ylim(0, 108); ax.legend(loc="upper left", fontsize=9)
    ax.set_title("Fig 7.  Results (intel mode) - 17/17, undamaged. Kills 10 (enemy HP 0), Judged 7, evaders A3/D2* defeated",
...
def _run_traj(opp_name, tac_fn, dur=120.0):
    ...
    def fn(o):
        ...
        dx, dy = on-en, oe-ee; h = math.radians(ob.ego_psi_deg)
...
def fig3_shapes():
    ...
    rf, tac = _train(DS_DA)
    cases = [("anchor_aggressive", "kill (spiral-in)", RED), ("C2_OneCircleRad", "kill (hook-in)", RED),
             ("A3_LagAngler", "draw (standoff arc)", PURPLE), ("D2_LastDitch", "draw (wide orbit)", PURPLE)]
    ...
    fig.suptitle("Fig 3.  Enemy relative trajectory shape (us = center triangle). "
                 "Kills spiral into center; evaders hold a wide orbit.", fontsize=11.5, x=0.01, ha="left", y=1.02)
...
def fig6_d2_sequence():
    ...
    ax[0].set_ylabel("range (ft)"); ax[0].text(2, 3300, "WEZ range band", color=GREEN, fontsize=8)
    ax[1].plot(rec["t"], rec["ua"], color=BLUE, lw=1.4, label="our altitude")
    ax[1].plot(rec["t"], rec["ea"], color=RED, lw=1.4, label="D2 altitude")
    ax[1].set_ylabel("altitude (ft)"); ax[1].set_xlabel("t (s)"); ax[1].legend(fontsize=9)
    for i, nm in enumerate(names):
        x = i*DUR/N
        for a in ax: a.axvline(x, color="#bbb", ls=":", lw=0.8)
        ax[0].text(x+1, ax[0].get_ylim()[1]*0.93, nm, fontsize=8, color=PURPLE)
    fig.suptitle(f"Fig 6.  D2 winning 6-phase sequence - range & altitude (result HP 100:{res.health2:.0f}, undamaged judged win)",
                 fontsize=11.5, x=0.01, ha="left", y=0.98)
...
def fig4_separability():
    ...
    from exp_e27_adaptive_subset import AdaptivePolicy  # noqa
    from exp_e7_champion import _train
    from exp_e10_unified import DS_DA
    from lqr import GainScheduledLQR
    from autopilot import AutopilotConfig
    ...
    for opp in FULL:
        ...
        d = opp in DRAWS
        ax.scatter(f["reopen"], f["aa_min"], s=120 if d else 60,
    ...
    ax.text(700, 33, "A3: reopen<3000", color=PURPLE, fontsize=8.5)
    ax.text(7800, 33, "D2: aa_min>30 & rmin>3000", color=RED, fontsize=8.5)
    ax.set_xlabel("reopen (ft) - range re-opening after closest approach")
    ax.set_ylabel("aa_min (deg) - minimum aspect angle")
    ax.set_title("Fig 4.  Shape-feature separability - draw evaders (purple star) split from 15 wins (false positive 0)",
                 fontsize=10.5, loc="left")
    fig.savefig(os.path.join(FIG, "fig4_separability.png")); plt.close(fig)


if __name__ == "__main__":
    print("schematic figs..."); fig1_architecture(); fig2_wez_geometry(); fig5_etm_concept(); fig8_deadlock(); fig7_results()
    print("real-data figs (engine)...")
    fig3_shapes(); print("  fig3 ok")
    fig4_separability(); print("  fig4 ok")
    fig6_d2_sequence(); print("  fig6 ok")
    print("done:", FIG); print(os.listdir(FIG))
