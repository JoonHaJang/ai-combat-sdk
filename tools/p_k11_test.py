"""R15-K K11 spawn-phase merge lead turn 측정."""
...
SETTINGS = ["K2,K8,K10", "K2,K8,K10,K11", "K11"]
RUNS = 5
...
def main():
    ...
    for ks in SETTINGS:
        ...
        for opp in OPPS:
            ...
            print(f"{ks:<16} {opp:<14} mean={statistics.mean(deals):>5.2f} ±{statistics.stdev(deals) if len(deals)>1 else 0:>4.2f}  "
                  f"taken={statistics.mean(takens):>5.2f}  WIN={wins}/{RUNS}", flush=True)
    print("\n=== SUMMARY (mean dmg / mean taken) ===")
    print(f"{'opp':<14} " + " ".join(f"{ks:>17}" for ks in SETTINGS))
    for opp in OPPS:
        cells = []
        for ks in SETTINGS:
        ...
        print(f"{opp:<14} " + " ".join(f"{c:>17}" for c in cells))
