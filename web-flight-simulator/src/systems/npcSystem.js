...
export class NPCSystem {
	...
	update(dt, playerPos) {
		...
		for (let i = this.npcs.length - 1; i >= 0; i--) {
			...
			// 외부 제어(JSBSim WS) NPC: AI 로직 건너뛰고 행렬만 업데이트
			if (npc.isExternal) {
				this._updateMeshMatrix(npc, viewMatrix);
	...
	updateExternal(id, lon, lat, alt_m, heading, pitch, roll) {
		// id로 기존 외부 NPC 찾거나 없으면 생성
		let npc = this.npcs.find(n => n.id === id);
		if (!npc) {
			if (!this.loaded) return;
			npc = this.createNPCMesh(id, lon, lat, alt_m, heading, 0);
			if (!npc) return;
			npc.id = id;          // createNPCMesh의 랜덤 suffix 덮어씀
			npc.isExternal = true;
		}
		npc.lon     = lon;
		npc.lat     = lat;
